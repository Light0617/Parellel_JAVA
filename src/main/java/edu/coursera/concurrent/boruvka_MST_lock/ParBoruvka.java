package edu.coursera.concurrent.boruvka_MST_lock;

import edu.coursera.concurrent.boruvka_MST_lock.boruvka.Edge;
import edu.coursera.concurrent.boruvka_MST_lock.boruvka.Component;
...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        ParComponent cur = null;
        while(!nodesLoaded.isEmpty()){
            cur = nodesLoaded.poll();
            if(cur == null || !cur.lock.tryLock()) continue;
            if(cur.isDead) {
                cur.lock.unlock();
                continue;
            }

            final Edge<ParComponent> edge = cur.getMinEdge();
            if(edge == null) {
                cur.lock.unlock();
                solution.setSolution(cur);
                break;
            }

            final ParComponent other = edge.getOther(cur);
